    ...
    try {
        ...
        if (userDoc) {
            ...
            if (passOK) {
                ...
                res.cookie('token', token, { httpOnly: false, secure: true, sameSite: "none", domain: "hotel-dev-backend.onrender.com", expires: new Date(Date.now() + 5 * 60 * 60 * 1000) }).json({ mymessage: "ok", data2: token, userDoc });
            } else {
